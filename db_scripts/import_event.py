...
def fast_ingest():
    ...
    logging.info(f"📊 databasealready有 {existing_count:,} 条记录")
    logging.info("-" * 60)
    ...
    for i, file in enumerate(csv_files):
        ...
        try:
            ...
            # 把all缺失orerror坐标，统一流放到 "Null Island" (0.0, 0.0)
            ...
            logging.info(f"   ⚡ 呼叫底层 LOAD DATA 指令灌入 MySQL... ({row_count:,} row)")
    ...
    logging.info(f"   skip（already存在）: {skipped_count} 个file")
