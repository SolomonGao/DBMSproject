...
- [x] 3.1 开发6个核心工具 → `mcp_server/app/tools/core_tools_v2.py`
...
### Phase 3.5: 工具补充增强 (1周) ⏳ 待开始
- [ ] 3.5.1 `track_theme` - 主题追踪（追踪"俄乌冲突"等主题发展）
- [ ] 3.5.2 `compare_regions` - 区域对比（对比多国冲突趋势）
- [ ] 3.5.3 `execute_sql` - 自定义查询（复杂统计、数据导出预览）
- [ ] 3.5.4 `export_events` - 数据导出（CSV/JSON格式）

**补充说明**:
当前6个工具覆盖约 **80%** 日常需求，补充后可达 **95%**:
| 用户类型 | 当前覆盖 | 补充后覆盖 | 还需工具 |
|---------|---------|-----------|---------|
| 普通分析师 | 90% | 95% | `track_theme` |
| 数据科学家 | 50% | 90% | `execute_sql` + `export_events` |
| 主题研究员 | 60% | 95% | `track_theme` + `compare_regions` |
...
| `daily_summary` | 366 | 每日摘要（2024全年，闰年） | ✅ 已完成 |
